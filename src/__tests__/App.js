...
describe("<App />", () => {
  ...
  it("Changes the folderItemElement and toggleInformation in state on click handler", () => {
    ...
    const props = {
      ...
      handleClick: index => {
        const toggle =
          index !== wrapper.state().folderItemElement
            ? true
            : !wrapper.state().toggleInformation;
        wrapper.setState({
  ...
  it("Changes the state with the handleSubmit instance", () => {
    ...
    filterWrapper.find("input").simulate("change", { target: { value: "A" } });
    expect(wrapper.state().filter).toBe("A");
    filterWrapper.find("input").simulate("change", { target: { value: "AB" } });
    expect(wrapper.state().filter).toBe("AB");
  });
  ...
  it("Changes the state with the sortFolder instance", () => {
    ...
    const props = {
      sortFolders: value => {
        const newReverse =
          value !== wrapper.state().sortValue
            ? false
            : !wrapper.state().columnReverse;
        wrapper.setState({
          sortValue: value,
