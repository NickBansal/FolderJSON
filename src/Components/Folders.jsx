...
const FoldersOrFiles = ({
  ...
  const filteredData = !filter
    ...
    : FoldersData.filter(item =>
        RegExp(filter.toLowerCase().trim()).test(item.name.toLowerCase().trim())
      );

  const folderSort = !sortValue
    ? filteredData
    : sortValue === "added"
    ? filteredData.sort(dateSort)
    : sortValue === "name"
    ? filteredData.sort(nameSort)
    : filteredData.sort(sizeSort);
  ...
  return (
    <div style={style} className="Folders">
      {folderSort.map((folder, index) => {
        const arrow =
          toggleInformation && folderItemElement === index ? "down" : "right";
        ...
        return (
          <div className="Folders__files" key={folder.name}>
            <div className="Folders__item">
              <div className="Folders__image">
                <img src={imageSource} alt={folder.type} />
                <p>{folder.size}mb</p>
              </div>
              ...
              {folder.type === "folder" && (
                <i
                  ...
                  className={`fas fa-chevron-${arrow} fa-2x`}
                />
              )}
            </div>
